...
void drawCageBar(){
	...
	glTranslatef(4, 0, 0);
	glRotatef(90,0,1,0);
    glutSolidSphere(1, 32, 32 );	//First end ball
    glPopMatrix();
	glPushMatrix();
	glColor3f(255,0,0);
	glTranslatef(-4,0,0);
	glRotatef(90,0,1,0);	
	glutSolidSphere(1,32,32);		//Second end ball
	glPopMatrix();
	glPushMatrix();
	glColor3f(0,0,255);
	glRotatef(90,1,0,0);
	glScalef(1.5,0.1,0.1);
	glutSolidCylinder( 3, 5, 32, 8 );	//Cylinder joining the two balls
	glPopMatrix();
}

void drawCageSquare(){					//Draws four of the above cage bars to make a square
	glPushMatrix();
	glTranslatef(0,4,0);
 	drawCageBar();
	glPopMatrix();
...
void drawCage(){					//Uses 6 of the cage squares to form a cube
	glPushMatrix();
	glTranslatef(0,0,-4);
	drawCageSquare();
	glPopMatrix();
...
void draw() {
	...
	if(objectNumber == 1){
		...
		glBegin(GL_LINE_LOOP);
		for(int i = 0; i < sizeof(dodecTriangles)/sizeof(dodecTriangles[0]); i++){	//Loops over the triangles
			for (int j = 0; j < sizeof(dodecVertices)/sizeof(dodecVertices[0]); j++){	//Loops over the triangle vertices
				glVertex3f(dodecVertices[dodecTriangles[i][j]][0],dodecVertices[dodecTriangles[i][j]][1],dodecVertices[dodecTriangles[i][j]][2]);
			}
		}
		glEnd();
